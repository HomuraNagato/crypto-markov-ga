...
df <- btc_usd %>% mutate(date = anytime(timestamp))
df <- df %>% mutate(fdate = factor(format(date, '%Y-%m-%d')))

# get daily closing price
# october 11th
# 51682386, 1539316798, 6189.99, 0.04746211, 2018-10-11 23:59:58, 2018-10-11 2
daily_btc <- df %>% group_by(fdate) %>% slice(c(n()))
tail(daily_btc) # it worked as inteded :)
